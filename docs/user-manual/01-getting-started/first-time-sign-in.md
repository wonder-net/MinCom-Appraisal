...
roles: [all]
public: true
keywords: [invitation, temporary password, first login, change password, MFA setup, two-factor, welcome email, forced password change, new user]
summary: Complete your first sign-in using the temporary password from your invitation email, change it to a permanent one, and set up MFA if required.
---
...
![MINCOM Appraisal sign-in page showing empty email and password fields](../../screenshots/employee/10-login-empty.png)

### Step 2: Enter your email and temporary password
...
![Sign-in page with email address and temporary password filled in, Sign in button highlighted](../../screenshots/employee/10-login-credentials-filled.png)
...
|---------|-------|----------|
| "Invalid credentials" on first sign-in | Typo or expired temp password | Copy the password directly from the invitation email; ask HR to resend if expired |

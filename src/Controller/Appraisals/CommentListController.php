...
/**
 * Port of CommentViewSet.list(). Read access: appraisee, their manager,
 * HR Admin, Executive (AppraisalRepository::canUserRead).
 */
final class CommentListController
{
    ...
    #[Route('/api/v1/appraisals/{appraisalId}/comments/', name: 'appraisal_comments_list', methods: ['GET'], requirements: ['appraisalId' => '[0-9a-fA-F-]{36}'])]
    public function __invoke(string $appraisalId, Request $request, #[CurrentUser] User $user): JsonResponse
    {
        ...
        $result = $this->comments->findByAppraisalPaginated($appraisal, $page, $pageSize);

        return new JsonResponse([
            'results' => array_map(fn (Comment $c) => $this->responseBuilder->build($c), $result['items']),
